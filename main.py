...
if uploaded_file is not None:
    ...
    def contains_two_numbers(text):
        """Check if a string contains exactly two numbers."""
        return bool(re.fullmatch(r"\d{2}", text))

    def calculate_productivity_per_cycle(df):
        df['Service No.'] = df['Service No.'].astype(str)
        df['Cycle'] = df['Service No.'].apply(lambda x: x if contains_two_numbers(x) else "NO IDENTIFIER OF CYCLE")
