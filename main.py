...
if uploaded_file is not None:
    ...
    with col1:
        ...
        # Corrected date range input
        start_date, end_date = st.date_input("Select date range", min_value=min_date, max_value=max_date)
        ...
        for (date, collector), collector_group in filtered_df[~filtered_df['Remark By'].str.upper().isin(['SYSTEM'])].groupby([filtered_df['Date'].dt.date, 'Remark By']): 
            ...
            total_connected = round(total_connected, 2)
            total_ptp = round(total_ptp, 2)
            total_rpc = round(total_rpc, 2)
        ...
        total_row = pd.DataFrame([{
            'Day': 'Total',
            'Collector': '',
            'Total Connected': round(collector_summary['Total Connected'].sum(), 2),
            'Total PTP': round(collector_summary['Total PTP'].sum(), 2),
            'Total RPC': round(collector_summary['Total RPC'].sum(), 2),
    ...
    with col2:
        ...
        for cycle, cycle_group in df.groupby('Service No.'):
            ...
            totals_row_cycle = {
                'Cycle': cycle,
                'Time Interval': 'Total',
                'Total Connected': round(cycle_time_summary['Total Connected'].sum(), 2),
                'Total PTP': round(cycle_time_summary['Total PTP'].sum(), 2),
                'Total RPC': round(cycle_time_summary['Total RPC'].sum(), 2),
