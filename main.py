...
if uploaded_file is not None:
    ...
    with col2:
        ...
        # Group by 'Date' and 'Service No.' (Cycle)
        for (date, cycle), cycle_group in filtered_cycle_df.groupby([filtered_cycle_df['Date'].dt.date, 'Service No.']):
            # Calculate the metrics
        ...
        # Calculate and append totals for the cycle summary
        total_row_cycle = pd.DataFrame([{
            'Day': 'Total',
            'Cycle': '',
        ...
        cycle_summary = pd.concat([cycle_summary, total_row_cycle], ignore_index=True)
