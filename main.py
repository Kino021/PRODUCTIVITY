...
if uploaded_file is not None:
    ...
    with col1:
        ...
        # Initialize the summary table by Collector
        collector_summary = pd.DataFrame(columns=[
            'Collector', 'Time Interval', 'Total Calls', 'Total Connected', 'Total PTP', 'Total RPC', 'PTP Amount', 'Balance Amount'
        ])

        # Group the data by Collector and calculate the metrics
        for collector, collector_group in df.groupby('Collector'):
            # Group by Time Interval within the current collector's data
            for time_interval, time_interval_group in collector_group.groupby('Time Interval'):
        ...
        collector_summary['Time Interval'] = pd.Categorical(
            collector_summary['Time Interval'], categories=time_interval_sort_order.keys(), ordered=True)
        collector_summary = collector_summary.sort_values('Time Interval')
